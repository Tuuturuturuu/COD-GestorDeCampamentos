...
public class NFacturaTest {
	...
	@Test
	public void CerrarVenta() throws NumberFormatException, ParseException {
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		int idCliente = 1;
		TFactura tFacturaExp = new TFactura(1, idCliente, format.parse(Instant.now().toString()), Float.parseFloat("5"),
				false);
	...
	@Test
	public void devolucionVenta() throws NumberFormatException, ParseException {
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
		SAFactura saFactura = FactoriaSAImp.obtenerInstancia().generarSAFactura();
		TFactura tFacturaExp = new TFactura(1, 1, format.parse(Instant.now().toString()), Float.parseFloat("5"), true);

		TFactura tFacturaAct = new TFactura(1, 1, format.parse(Instant.now().toString()), Float.parseFloat("5"), false);
		Set<TLineaFactura> LineasFacturaAct = new HashSet<TLineaFactura>();
		TLineaFactura linea = new TLineaFactura(1, 1, Float.parseFloat("5"), 10);
		LineasFacturaAct.add(linea);
		TCarrito actual = new TCarrito(tFacturaAct, LineasFacturaAct);
