...
public class VVincularActividadConMaterial extends JFrame implements IGUI {
	...
	private void initGUI() {
		...
		JLabel msgIntroIDCabecera = ComponentsBuilder.createLabel(
				"Introduzca los IDs de la actividad que quieres vincular y el material correspondiente", 1, 10, 80, 20,
				Color.BLACK);
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Controlador.obtenerInstancia()
							.run(new TMaterial(!matId.getText().isEmpty() ? Integer.parseInt(matId.getText()) : 0, null,
									null, null, !actId.getText().isEmpty() ? Integer.parseInt(actId.getText()) : 0,
									null), Evento.EVincularMaterialActividadOK);
				} catch (Exception ex) {
					Controlador.obtenerInstancia().run(new TMaterial(-38, null, null, null, null, null),
							Evento.EVincularMaterialActividadOK);
				}
			}
		});
