...
public class VMostrarFactura extends JFrame implements IGUI {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					Controlador.obtenerInstancia().run(Integer.parseInt(id.getText()),
							Evento.EMostrarActividadesporFacturaOK);
				} catch (Exception ex) {
					Controlador.obtenerInstancia().run(-38, Evento.EMostrarActividadesporFacturaOK);
				}
	...
	public void actualizar(Object object, Evento event) {
		...
		Set<TLineaFactura> listaCarritos = aux.gettGenera();
		JPanel mainPanel = new JPanel();
		mainPanel.setLayout(new BoxLayout(mainPanel, BoxLayout.Y_AXIS));
		this.setContentPane(mainPanel);
