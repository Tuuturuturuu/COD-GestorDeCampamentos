...
public class SAActividadImp implements SAActividad {
	...
	private DAOActividadMaterial daoActividadMaterial = FactoriaIntegracionImp.obtenerInstancia()
			.generaDAOActividadMaterial();
	...
	public TActividad crearActividad(TActividad tActividad) {
		...
		if (tActividad.getIdActividad() >= 0) {
			if (tActividad.getLugar().isEmpty() || tActividad.getNombre().isEmpty() || tActividad.getNumplazas() == 0
					|| tActividad.getIdPersonal() == 0)
	...
	public TActividad modificarActividad(TActividad tActividad) {
		...
		if (tActividad.getIdActividad() >= 0) {
			...
			// se quiere cambiar idPersonal Y DEBERIAMOS COMPROBAR QUE ID DE
			// EMPLEADO EXISTA EN LA BASE DE DATOS
			if (tActividad.getIdPersonal() != 0 && tActividad.getIdActividad() > 0) {
				if (daoPersonal.MostrarUno(tActividad.getIdPersonal()).getIdPersonal() == -1) // Comprobar
																								// que
																								// el
																								// id
																								// de
	...
	public TActividad borrarActividad(TActividad tActividad) {
		...
		if (tActividadbbdd.getIdActividad() != -1) {
			if (tActividadbbdd.getActivo() == true) {
				// Conseguimos todos los elementos ActividadMaterial que tengan
				// el idActividad en com�n
				...
				for (TActividadMaterial actividadMaterial : ActividadesMaterial) {
					// Desvinculamos pasando el id del Material y de la
					// Actividad para asegurarnos que es correcto
					correct = daoActividadMaterial.desvincular(actividadMaterial.getIdActividad(),
							actividadMaterial.getIdMaterial());
					if (correct == 1)
						i++;
				}
