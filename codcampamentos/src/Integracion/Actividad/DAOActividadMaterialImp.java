/**
 * 
 */
package Integracion.Actividad;

import Negocio.Actividad.TActividadMaterial;
import java.util.Set;

/** 
* <!-- begin-UML-doc -->
* <!-- end-UML-doc -->
* @author airam
* @generated "UML a Java (com.ibm.xtools.transform.uml2.java5.internal.UML2JavaTransform)"
*/
public class DAOActividadMaterialImp implements DAOActividadMaterial {
	/** 
	* (non-Javadoc)
	* @see DAOActividadMaterial#vincular(TActividadMaterial actividadMaterial)
	* @generated "UML a Java (com.ibm.xtools.transform.uml2.java5.internal.UML2JavaTransform)"
	*/
	public Integer vincular(TActividadMaterial actividadMaterial) {
		// begin-user-code
		// TODO Auto-generated method stub
		return null;
		// end-user-code
	}

	/** 
	* (non-Javadoc)
	* @see DAOActividadMaterial#desvincular(Integer idActividad, Integer idMaterial)
	* @generated "UML a Java (com.ibm.xtools.transform.uml2.java5.internal.UML2JavaTransform)"
	*/
	public Integer desvincular(Integer idActividad, Integer idMaterial) {
		// begin-user-code
		// TODO Auto-generated method stub
		return null;
		// end-user-code
	}

	/** 
	* (non-Javadoc)
	* @see DAOActividadMaterial#BuscarporActividad(Integer idActividad)
	* @generated "UML a Java (com.ibm.xtools.transform.uml2.java5.internal.UML2JavaTransform)"
	*/
	public Set<TActividadMaterial> BuscarporActividad(Integer idActividad) {
		// begin-user-code
		// TODO Auto-generated method stub
		return null;
		// end-user-code
	}

	/** 
	* (non-Javadoc)
	* @see DAOActividadMaterial#BuscarporMaterial(Integer idMaterial)
	* @generated "UML a Java (com.ibm.xtools.transform.uml2.java5.internal.UML2JavaTransform)"
	*/
	public Set<TActividadMaterial> BuscarporMaterial(Integer idMaterial) {
		// begin-user-code
		// TODO Auto-generated method stub
		return null;
		// end-user-code
	}

	/** 
	* (non-Javadoc)
	* @see DAOActividadMaterial#BuscarActividadMaterial(Integer idActividad, Integer idMaterial)
	* @generated "UML a Java (com.ibm.xtools.transform.uml2.java5.internal.UML2JavaTransform)"
	*/
	public TActividadMaterial BuscarActividadMaterial(Integer idActividad, Integer idMaterial) {
		// begin-user-code
		// TODO Auto-generated method stub
		return null;
		// end-user-code
	}
}